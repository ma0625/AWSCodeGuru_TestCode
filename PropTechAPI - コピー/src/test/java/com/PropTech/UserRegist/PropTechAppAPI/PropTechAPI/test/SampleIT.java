package com.PropTech.UserRegist.PropTechAppAPI.PropTechAPI.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        File f = new File("./target/PropTechAPI.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "PropTechAPI.war").importFrom(f).as(WebArchive.class);
        
        // Add the package containing the test classes:
        war.addPackage("com.PropTech.UserRegist.PropTechAppAPI.PropTechAPI.test");
